...
@jit(nopython=True)
def APLB_acc_LCApril_3D(lambda_0, s, a, P, Psi_0, phi_2, phi_3, t_0, z_0, x_0, y_0, beta_0, tau_p):
    ...
    n = (lambda_0/(0.8e-6))*1000  # np.maximum(50, np.round(np.sqrt(P/(w_0**2))/(5e10)))  # empirically chosen resolution based on field strength
    ...
    for k in range(0, len(time)-1):
        ...
        Rt = np.sqrt(r**2 + (z[k] + 1j*a)**2)
        Rtomega = np.sqrt(r**2 + (z[k] - z_omega + 1j*a)**2)

        B_z_spec = -1*pulse_prep*(2*1j*Amp*np.exp(-omega*a/c)/(Rtomega)**2)*(np.sin(omega*Rt/c)*((2+(omega*r/c)**2)/Rtomega - 3*r**2/Rtomega**3)+np.cos(omega*Rt/c)*(3*omega*r**2/(Rtomega**2*c)-2*omega/c))/c
        
        B_z_time = np.sum(B_z_spec*np.exp(1j*omega*time[k]))*omega_step/(delta_omega*np.sqrt(np.pi))
        B_z_total = np.real(np.exp(1j*(Psi_0+np.pi/2))*B_z_time)
        
        B_x_spec = -1*pulse_prep*(2*1j*Amp*np.exp(-omega*a/c))*(r*(z[k] - z_omega + 1j*a)/Rtomega**3)*(np.sin(omega*Rt/c)*(3/Rtomega**2 - (omega/c)**2) - 3*omega*np.cos(omega*Rt/c)/(Rtomega*c))*(x[k]/r)/c
        
        B_x_time = np.sum(B_x_spec*np.exp(1j*omega*time[k]))*omega_step/(delta_omega*np.sqrt(np.pi))
        B_x_total = np.real(np.exp(1j*(Psi_0+np.pi/2))*B_x_time)
        
        B_y_spec = -1*pulse_prep*(2*1j*Amp*np.exp(-omega*a/c))*(r*(z[k] - z_omega + 1j*a)/Rtomega**3)*(np.sin(omega*Rt/c)*(3/Rtomega**2 - (omega/c)**2) - 3*omega*np.cos(omega*Rt/c)/(Rtomega*c))*(y[k]/r)/c
        ...
        E_x_spec = pulse_prep*(2*1j*Amp*np.exp(-omega*a/c))*(1j*omega*r/(c*Rtomega)**2)*(np.sin(omega*Rt/c)/Rtomega - omega*np.cos(omega*Rt/c)/c)*(-y[k]/r)*c
        ...
        E_y_spec = pulse_prep*(2*1j*Amp*np.exp(-omega*a/c))*(1j*omega*r/(c*Rtomega)**2)*(np.sin(omega*Rt/c)/Rtomega - omega*np.cos(omega*Rt/c)/c)*(x[k]/r)*c
        
        E_y_time = np.sum(E_y_spec*np.exp(1j*omega*time[k]))*omega_step/(delta_omega*np.sqrt(np.pi))
        E_y_total = np.real(np.exp(1j*(Psi_0+np.pi/2))*E_y_time)
